...
export const CardMaker = () => {
  const { images, sets, layout } = useContext(ImagesContext)
  ...
  const cards = sets.map(( set, setIndex ) => {
    const pictures = set.map(( imageIndex, layoutIndex ) => {      
      ...
      if (imageData) {
        const href = URL.createObjectURL(imageData)
        ...
        let angle = (Math.atan(cy / cx) / Math.PI * 180) - 90
        ...
        const defId = `crop-circle-${setIndex * 20 + imageIndex}`
        const pictureData = {
          ...layoutData,
          href,
          defId,
